...
def filter_paths(paths: list[str], excludes: list[str] | None = None) -> list[str]:
    patterns = [pattern for pattern in (excludes or []) if pattern]
...
def has_changes(repo_dir: str | Path, excludes: list[str] | None = None) -> bool:
    return bool(filter_paths(get_changed_files(repo_dir), excludes))
